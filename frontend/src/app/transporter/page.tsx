...
import { Truck, MapPin, RefreshCw, Navigation, Package } from 'lucide-react';
...
const TransporterDashboardComponent: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (data && data.batches) {
        // Show batches that are relevant to transporter (waiting at mandi or currently in transport)
        const transportBatches = data.batches.filter((b: any) => 
          ['mandi', 'transport'].includes(b.currentStage?.toLowerCase())
        );
  ...
  const handleStartTransport = async (batchId: string) => {
  ...
  const waitingCount = batches.filter(b => b.currentStage?.toLowerCase() === 'mandi').length;
  const inTransitCount = batches.filter(b => b.currentStage?.toLowerCase() === 'transport').length;
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-border/40 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-sm text-muted-foreground">Track shipments and update delivery status</p>
          </div>
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Card className="border border-border bg-card">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground mb-1">Waiting for Pickup</p>
              <h2 className="text-3xl font-bold text-foreground">{waitingCount}</h2>
            </div>
        ...
        <Card className="border border-border bg-card">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground mb-1">In Transit</p>
              <h2 className="text-3xl font-bold text-foreground">{inTransitCount}</h2>
            </div>
      ...
      <Card className="border border-border bg-card shadow-sm h-full">
        <CardHeader className="pb-3 border-b border-border/40 bg-muted/20">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-foreground">Assigned Batches</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {batches.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground flex flex-col items-center">
              <Truck className="h-12 w-12 mb-3 text-muted" />
              <p>No batches assigned to you currently.</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {batches.map((batch) => (
                    <TableRow key={batch.batchId || batch._id} className="border-b border-border/40 hover:bg-muted/20 text-left transition-colors">
                      ...
                      <TableCell className="py-4 px-6 text-muted-foreground flex items-center gap-1">
                      ...
                      <TableCell className="py-4 px-6 text-right">
                        {batch.currentStage?.toLowerCase() === 'mandi' ? (
                          <Button 
                            size="sm" 
                            className="bg-amber-600 hover:bg-amber-700 text-white"
                            onClick={() => handleStartTransport(batch.batchId || batch._id)}
                            disabled={updatingId === (batch.batchId || batch._id)}
                          >
                            {updatingId === (batch.batchId || batch._id) ? 'Updating...' : 'Start Transit'}
                          </Button>
                        ) : batch.currentStage?.toLowerCase() === 'transport' ? (
                          <Button 
                            size="sm" 
                            className="bg-purple-600 hover:bg-purple-700 text-white"
                            onClick={() => handleDeliverBatch(batch.batchId || batch._id)}
                            disabled={updatingId === (batch.batchId || batch._id)}
                          >
                            {updatingId === (batch.batchId || batch._id) ? 'Delivering...' : 'Mark Delivered'}
                          </Button>
                        ) : null}
                      </TableCell>
                    </TableRow>
